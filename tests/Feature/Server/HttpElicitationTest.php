...
it('can complete an elicitation round trip over streamable http', function (): void {
    ...
    try {
        ...
        cache()->put("mcp:response:{$sessionId}:{$elicitationRequestId}", json_encode([
            'jsonrpc' => '2.0',
...
it('returns a clear error when an http elicitation request is not streaming', function (): void {
    ...
    $response->assertStatus(200);
    $response->assertJsonPath('error.message', 'A server-to-client request requires a text/event-stream response. Send the request with Accept: text/event-stream.');
});
